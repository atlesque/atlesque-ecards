<?php
function atlesque_ecard_help_page()
{
	...
	<div class="wrap">
		...
		if($title=='') {
			...
			if(isset($options['atlesque_ecard_select_pickup_field'])) {
				...
				echo "<li>It is compulsary to set Pickup Page from Basic Settings. Pickup page should have shortcode [atlesque-ecard-pick] as content.</li>";
			}
			...
			echo "</ul>";
			echo "<h3>Tips</h3><ul>";
			echo '<li><b>To Display All Ecard:</b> Create a page and add it to menu. The content should be  [atlesque-ecard-list perrow="3" perpage="30" orderby="date" page="off" layout="list"] </li>';
			echo '<li><b>To create pickup page: </b>Create a page and set content as [atlesque-ecard-pick]</li>';
			...
			<form class="pure-form" method="post" action="">
				<table class="form-table"><tbody><tr><th scope="row">Subject</th>
					<td>
						<input name="subject" id="subject" type="text" value="" class="regular-text code" required>
					</td>
				</tr>
				<tr>
				  ...
				  wp_editor('', 'odudecardcontent', apply_filters('atlesque_ecard_editor_settings', $settings)); ?>
				</div>
			</div>
...
echo "<ul><li>Name: $name </li><li>Username: $username</li><li>Email: $email</li><li>URL: $url</li></ul>";
	...
	//Sending Mail
	add_filter( 'wp_mail_content_type', 'atlesque_ecard_html_content_type' );
	$headers[] = 'From: '.$name.' <'.$email.'>';
	wp_mail( 'navneet@odude.com', $sub, $msg, $headers );
	// Reset content-type to avoid conflicts
	remove_filter( 'wp_mail_content_type', 'atlesque_ecard_html_content_type' );
